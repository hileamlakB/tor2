...
class DirectoryServerServicer(tor_pb2_grpc.DirectoryServerServicer):
    def GetRelayNodes(self, request, context):
        ...
        entry_node = tor_pb2.RelayNode(
            address="localhost:2525", node_type=tor_pb2.ENTRY)
        middle_node = tor_pb2.RelayNode(
            address="localhost:2526", node_type=tor_pb2.MIDDLE)
        exit_node = tor_pb2.RelayNode(
            address="localhost:2527", node_type=tor_pb2.EXIT)
